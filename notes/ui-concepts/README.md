...
| --- | --- | --- |
| Brush / eraser | B / E | Built-in brush buttons / Eraser |
| Size | Shift + horizontal drag; [ / ] | Visible Size slider |
| Opacity | Hold O + vertical drag; Shift+[ / ] | Visible Opacity slider |
| Pan | Hold Space + drag; middle drag; trackpad pixel scroll | Pan, then drag |
| Zoom / fit | Shift+Space + drag; wheel; platform pinch; Home | − / +; tap percentage to fit |
...
| Focus / help | Tab or F1 / ? | Focus / ?; Show tools restores UI |

The earlier puck has been replaced by separate visible sliders. Keyboard canvas drags retain their axis locking: size doubles per 100 logical pixels; opacity changes by 50 percentage points per 100 vertical pixels. Up increases opacity. Canvas adjustments keep the brush preview anchored at the initial contact, with a fixed orientation and live size feedback. They retain ownership until release, even when the modifier is released first. Mouse release or Escape restores the actual pointer to the starting point on platforms that support pointer warping (including Atlas X11). Escape restores the initial canvas adjustment; cancelling a pen adjustment suppresses the remaining contact until lift.
