use crate::{
    ...
    gpu_history_recovery::{
        GpuHistoryRecoveryEntry, GpuHistoryRecoveryError, GpuHistoryRecoverySpills,
    },
...
impl Error for GpuLiveRecoveryError {
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            ...
            Self::PreparedSourceChanged { .. } => None,
        }
    }
}
...
mod tests {
    ...
    fn ready_state() -> (GpuLiveRecovery, GpuHistoryId) {
        ...
        let timeline = GpuRecoveryTimeline::new(reconciler.snapshot(), 8, 1_000_000).unwrap();
